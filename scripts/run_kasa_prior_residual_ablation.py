#!/usr/bin/env python3
"""Run KASA output prior-residual ablation on PeMS04 (full_prior_on vs woprior)."""
...
TEMP_CFG_DIR = ROOT / "tmp_configs" / "kasa_prior_residual_ablation"
LOG_DIR = ROOT / "results" / "kasa_prior_residual_ablation_logs"
CKPT_ROOT = ROOT / "checkpoints" / "kasa_prior_residual_ablation"

DEFAULT_VARIANTS = ["full_prior_on", "woprior"]
...
def generate_temp_config(variant: str, seed: int) -> Path:
    ...
    lines = [
        "",
        "# ===== kasa_prior_residual_ablation overrides (auto-generated) =====",
        f"CFG.ENV.SEED = {seed}",
        "if hasattr(CFG, 'SEED'):",
        f"    CFG.SEED = {seed}",
        "if hasattr(CFG, 'TRAIN') and hasattr(CFG.TRAIN, 'SEED'):",
        f"    CFG.TRAIN.SEED = {seed}",
        f'CFG.TRAIN.CKPT_SAVE_DIR = os.path.join("checkpoints", "kasa_prior_residual_ablation", "{variant}_seed{seed}")',
...
def build_paired_diff(rows: list[dict]) -> list[dict]:
    """Paired difference woprior - full_prior_on by seed. Positive = removing prior is worse."""
    ...
    for metric in ("mae", "rmse", "mape"):
        ...
        for seed in sorted({int(r["seed"]) for r in rows}):
            full = by_key.get(("full_prior_on", seed))
            wo = by_key.get(("woprior", seed))
            ...
            if fv is None or wv is None:
                continue
            diffs.append(float(wv) - float(fv))
...
def write_outputs(rows: list[dict], out_csv: Path, out_md: Path, summary_csv: Path):
    ...
    paired = build_paired_diff(rows)

    md = [
        "# KASA Prior Residual Ablation on PeMS04 12→12\n\n",
        "## Per-run results\n\n",
    ...
    md.append("\n## Paired difference: woprior - full_prior_on\n\n")
    md.append(
        "Positive value means removing prior residual is worse.\n\n"
    )
...
def main() -> int:
    parser = argparse.ArgumentParser(description="KASA output prior-residual ablation on PeMS04.")
    parser.add_argument("--variants", nargs="+", default=DEFAULT_VARIANTS,
                        choices=list(VARIANT_OVERRIDES.keys()))
    parser.add_argument("--seeds", type=int, nargs="+", default=DEFAULT_SEEDS)
    parser.add_argument("--gpus", nargs="+", default=["0"])
    parser.add_argument("--out", default="results/pems04_kasa_prior_residual_ablation.csv")
    parser.add_argument("--markdown", default="results/pems04_kasa_prior_residual_ablation.md")
    ...
    summary_csv = ROOT / "results" / "pems04_kasa_prior_residual_ablation_summary.csv"
