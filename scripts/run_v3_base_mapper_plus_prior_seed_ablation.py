#!/usr/bin/env python3
"""Run v3 base_mapper_plus_prior KAN vs MLP seed ablation with parallel GPU workers."""
...
TEMP_CFG_DIR = os.path.join(ROOT, "tmp_configs", "v3_base_mapper_plus_prior")
RUN_LOG_DIR = os.path.join(ROOT, "logs", "v3_base_mapper_plus_prior")
VARIANT_PREFIX = "v3_base_mapper_plus_prior"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Compare v3 base_mapper_plus_prior KAN vs MLP over multiple seeds."
    )
    parser.add_argument(
        "--cfg",
    ...
    parser.add_argument(
        "--out",
        default=os.path.join(ROOT, "results", "pems04_v3_base_mapper_plus_prior.csv"),
        help="CSV output path.",
    )
    parser.add_argument(
        "--markdown",
        default=os.path.join(ROOT, "results", "pems04_v3_base_mapper_plus_prior.md"),
...
def generate_temp_config(base_cfg: str, mapper: str, seed: int) -> str:
    ...
    ckpt_suffix = f"_v3_base_mapper_plus_prior_{mapper}_seed{seed}"
    override = f"""

# ===== v3 base_mapper_plus_prior ablation overrides (auto-generated) =====
CFG.MODEL.PARAM["residual_mode"] = "base_mapper_plus_prior"
...
def write_markdown(path: str, results: Sequence[RunResult], seeds: Sequence[int]) -> None:
    ...
    lines = ["# V3 Base Mapper Plus Prior Seed Ablation", ""]
