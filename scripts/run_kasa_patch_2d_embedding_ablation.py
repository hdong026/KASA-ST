#!/usr/bin/env python3
"""KASA patch 2D time-feature embedding ablation on PeMS04 12→12."""
...
TEMP_CFG_DIR = ROOT / "tmp_configs" / "kasa_patch_2d_embedding_ablation"
LOG_DIR = ROOT / "results" / "kasa_patch_2d_embedding_ablation_logs"
CKPT_ROOT = ROOT / "checkpoints" / "kasa_patch_2d_embedding_ablation"

DEFAULT_VARIANTS = ["serial_concat", "time_feature_2d", "time_feature_2d_small"]
...
VARIANT_SPECS: dict[str, dict] = {
    ...
    "time_feature_2d_small": {
        "patch_embedding_mode": "time_feature_2d",
        "patch_data_input_mode": "all",
        "patch_feature_dim": 16,
    },
}

PAIRED_DIFFS = [
    ("time_feature_2d", "serial_concat"),
    ("time_feature_2d_small", "serial_concat"),
    ("time_feature_2d_small", "time_feature_2d"),
]
...
def generate_temp_config(variant: str, seed: int) -> Path:
    ...
    lines = [
        "",
        "# ===== kasa_patch_2d_embedding_ablation overrides (auto-generated) =====",
        f"CFG.ENV.SEED = {seed}",
        "if hasattr(CFG, 'SEED'):",
        f"    CFG.SEED = {seed}",
        "if hasattr(CFG, 'TRAIN') and hasattr(CFG.TRAIN, 'SEED'):",
        f"    CFG.TRAIN.SEED = {seed}",
        f'CFG.TRAIN.CKPT_SAVE_DIR = os.path.join("checkpoints", "kasa_patch_2d_embedding_ablation", "{variant}_seed{seed}")',
...
def dry_run_info(variant: str, seed: int, cfg_path: Path) -> None:
    ...
    pfd = spec.get("patch_feature_dim")
    print(f"    patch_feature_dim: {pfd if pfd is not None else 'None (defaults to d_d)'}")
...
def main() -> int:
    parser = argparse.ArgumentParser(description="KASA patch 2D embedding ablation on PeMS04.")
    parser.add_argument("--variants", nargs="+", default=DEFAULT_VARIANTS, choices=list(VARIANT_SPECS.keys()))
    parser.add_argument("--seeds", type=int, nargs="+", default=DEFAULT_SEEDS)
    parser.add_argument("--gpus", nargs="+", default=["0"])
    parser.add_argument("--out", default="results/pems04_kasa_patch_2d_embedding_ablation.csv")
    parser.add_argument("--markdown", default="results/pems04_kasa_patch_2d_embedding_ablation.md")
