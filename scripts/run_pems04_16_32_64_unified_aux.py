#!/usr/bin/env python3
"""PeMS04 16/32/64 unified temporal-spatial auxiliary loss experiments."""
...
DEFAULT_WORK_DIR = ROOT / "experiments" / "pems04_unified_aux"
DEFAULT_LOG_ROOT = ROOT / "logs" / "pems04_unified_aux"
DEFAULT_CKPT_ROOT = ROOT / "checkpoints" / "pems04_unified_aux"
...
DEFAULT_VARIANTS = [
    ...
    "GR7_unified_residual_detach",
    "GR7_unified_mono",
    "GR14_two_level_sparse",
    "GR14_unified_residual_detach",
]
...
UNIFIED_AUX_DEFAULTS: dict[str, Any] = {
...
VARIANT_SPECS: dict[str, dict[str, Any]] = {
    ...
    "GR7_unified_direct_small": {
        **GR7_SPARSE_BASE,
        **UNIFIED_AUX_GR7,
        "unified_aux_loss_mode": "unified_direct_small",
    },
    "GR7_unified_residual_detach": {
        **GR7_SPARSE_BASE,
        **UNIFIED_AUX_GR7,
    ...
    "GR7_unified_mono": {
        **GR7_SPARSE_BASE,
        **UNIFIED_AUX_GR7,
        "unified_aux_loss_mode": "unified_mono",
    },
    "GR14_two_level_sparse": {
        **GR7_SPARSE_BASE,
...
def generate_temp_config(
    ...
    ckpt_rel = os.path.join(
        "checkpoints", "pems04_unified_aux", f"h{horizon}", variant, f"seed{seed}"
    )
    lines = [
        "",
        "# ===== pems04_unified_aux overrides (auto-generated) =====",
        f"CFG.ENV.SEED = {seed}",
        "if hasattr(CFG, 'SEED'):",
        f"    CFG.SEED = {seed}",
...
def validate_generated_config(cfg_path: Path, horizon: int, variant: str) -> None:
    ...
    sgw = list(param.get("spatial_graph_loss_weights") or [])
    if any(float(w) != 0.0 for w in sgw):
        raise ValueError(f"{cfg_path}: spatial_graph_loss_weights must not be enabled")
    aux_mode = str(param.get("unified_aux_loss_mode", "none")).lower()
    if aux_mode != str(spec.get("unified_aux_loss_mode", "none")).lower():
        raise ValueError(
            f"{cfg_path}: unified_aux_loss_mode expected {spec.get('unified_aux_loss_mode')}, "
            f"got {aux_mode}"
        )
    if aux_mode != "none":
        total_aux = (
            float(param.get("aux_eta_temporal", 0.05))
            + float(param.get("aux_temporal_anchor_weight", 0.02))
            + float(param.get("aux_eta_spatial", 0.03))
        )
        if total_aux > 0.15 + 1e-9:
            raise ValueError(
                f"{cfg_path}: unified auxiliary total weight {total_aux:.4f} exceeds 0.15"
            )
...
def compute_aux_weight_summary(spec: dict[str, Any], horizon: int) -> dict[str, Any]:
    hspec = horizon_spec(horizon)
    chain = list(hspec["chain_lengths"])
    h_full = float(chain[-1])
    mode = str(spec.get("unified_aux_loss_mode", "none"))
    eta_t = float(spec.get("aux_eta_temporal", 0.05))
    eta_s = float(spec.get("aux_eta_spatial", 0.03))
    gamma = float(spec.get("aux_temporal_anchor_weight", 0.02))
    q = float(spec.get("aux_temporal_power", 2.0))
    p = float(spec.get("aux_spatial_power", 2.0))
    mu: list[float] = []
    if mode != "none" and len(chain) >= 2:
        rhos = [float(chain[0]) / h_full, float(chain[1]) / h_full]
        denom = sum(r ** q for r in rhos)
        mu = [eta_t * (r ** q) / denom for r in rhos] if denom > 0 else [0.0, 0.0]
    nu: list[float] = []
    ratios = list(spec.get("graph_resolution_ratios") or [])
    if mode != "none" and ratios:
        stage_indices = list(range(max(len(ratios) - 1, 0)))
        aux_ratios = [float(ratios[i]) for i in stage_indices]
        denom = sum(r ** p for r in aux_ratios)
        nu = [eta_s * (r ** p) / denom for r in aux_ratios] if denom > 0 else []
    return {
        "unified_aux_loss_mode": mode,
        "mu_temporal": mu,
        "nu_spatial": nu,
        "gamma_anchor": gamma if mode != "none" else 0.0,
        "total_aux_cap": eta_t + gamma + eta_s if mode != "none" else 0.0,
    }
...
def base_row(
    ...
    hspec = horizon_spec(horizon)
    spec = variant_spec(variant)
    aux = compute_aux_weight_summary(spec, horizon)
    return {
        "horizon": horizon,
        ...
        "variant": variant,
        "seed": seed,
        "unified_aux_loss_mode": aux["unified_aux_loss_mode"],
        "mu_temporal": fmt_list(aux["mu_temporal"]),
        "nu_spatial": fmt_list(aux["nu_spatial"]),
        "gamma_anchor": aux["gamma_anchor"],
...
def write_outputs(rows: list[dict], out_csv: Path, out_md: Path) -> None:
    ...
    fields = [
        ...
        "variant", "seed", "unified_aux_loss_mode", "mu_temporal", "nu_spatial", "gamma_anchor",
        "spatial_placement", "post_spatial_mode",
    ...
    md = [
        "# PeMS04 16/32/64 Unified Temporal-Spatial Auxiliary Loss\n\n",
        "Protocol: INPUT_LEN=16, horizons 16/32/64, dyadic chain_lengths, "
        "KASA TemporalStep, use_prev_condition=True, no prior / no PTSE / no MTSR-P. "
        "Unified aux modes replace legacy equal-weight graph stage loss.\n\n",
        "## Per-run results\n\n",
        "| horizon | variant | seed | aux_mode | mu | nu | test MAE@best | test RMSE@best | "
        "best val MAE | status |\n",
        "|---:|---|---:|---|---|---|---:|---:|---:|---|\n",
    ]
    for r in rows:
        md.append(
            f"| {r['horizon']} | {r['variant']} | {r['seed']} | {r.get('unified_aux_loss_mode','')} | "
            f"{r.get('mu_temporal','')} | {r.get('nu_spatial','')} | "
            f"{fmt_val(r.get('test_mae_at_best_val'))} | {fmt_val(r.get('test_rmse_at_best_val'))} | "
            f"{fmt_val(r.get('best_val_mae'))} | {r.get('status','')} |\n"
        )

    md.append("\n## Per-horizon leaderboard (by test MAE@best-val)\n\n")
    horizons = sorted({int(s["horizon"]) for s in hv_summary})
    for horizon in horizons:
        md.append(f"### Horizon {horizon}\n\n")
        md.append("| rank | variant | MAE mean | MAE std | n |\n")
        md.append("|---:|---|---:|---:|---:|\n")
        h_rows = [s for s in hv_summary if int(s["horizon"]) == horizon]
        h_rows.sort(key=lambda s: math.inf if math.isnan(s["mae_mean"]) else s["mae_mean"])
        for i, s in enumerate(h_rows, 1):
            md.append(
                f"| {i} | {s['variant']} | {fmt_val(s['mae_mean'])} | "
                f"{fmt_val(s['mae_std'])} | {s['n']} |\n"
            )
        md.append("\n")

    md.append("## Per-horizon variant summary\n\n")
...
def dry_run_info(horizon: int, variant: str, seed: int, cfg_path: Path, ckpt_root: Path) -> None:
    ...
    print(f"    unified_aux_loss_mode: {spec.get('unified_aux_loss_mode')}")
    aux = compute_aux_weight_summary(spec, horizon)
    print(f"    mu_temporal: {aux['mu_temporal']}")
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="PeMS04 16/32/64 unified temporal-spatial auxiliary loss experiments."
    )
    ...
    parser.add_argument("--out", default="results/pems04_16_32_64_unified_aux.csv")
    parser.add_argument("--markdown", default="results/pems04_16_32_64_unified_aux.md")
