#!/usr/bin/env python3
"""Run core KASA ablations on PeMS04 official 6:2:2 protocol."""
...
TEMP_CFG_DIR = ROOT / "tmp_configs" / "kasa_622_core_ablation"
LOG_DIR = ROOT / "results" / "kasa_622_core_ablation_logs"
CKPT_ROOT = ROOT / "checkpoints" / "kasa_622_core_ablation"

DEFAULT_VARIANTS = ["final_mlp_off", "mlp_pre_on", "kan_pre_off", "woprior_pre_off"]
...
VARIANT_OVERRIDES: dict[str, dict] = {
    ...
    "kan_pre_off": {
        "prior_mapper_type": "kan",
        "use_pre_temporal_spatial_enhancement": False,
    },
    "woprior_pre_off": {
        "use_prior_residual": False,
        "use_pre_temporal_spatial_enhancement": False,
    },
}
...
def generate_temp_config(variant: str, seed: int) -> Path:
    ...
    lines = [
        "",
        "# ===== kasa_622_core_ablation overrides (auto-generated) =====",
        f"CFG.ENV.SEED = {seed}",
        "if hasattr(CFG, 'SEED'):",
        f"    CFG.SEED = {seed}",
        "if hasattr(CFG, 'TRAIN') and hasattr(CFG.TRAIN, 'SEED'):",
        f"    CFG.TRAIN.SEED = {seed}",
        f'CFG.TRAIN.CKPT_SAVE_DIR = os.path.join("checkpoints", "kasa_622_core_ablation", "{variant}_seed{seed}")',
...
def write_outputs(rows: list[dict], out_csv: Path, out_md: Path, summary_csv: Path):
    ...
    md = [
        "# PeMS04 6:2:2 KASA Core Ablation\n\n",
...
def main() -> int:
    parser = argparse.ArgumentParser(description="KASA core ablation on PeMS04 6:2:2.")
    parser.add_argument("--variants", nargs="+", default=DEFAULT_VARIANTS,
                        choices=list(VARIANT_OVERRIDES.keys()))
    parser.add_argument("--seeds", type=int, nargs="+", default=DEFAULT_SEEDS)
    parser.add_argument("--gpus", nargs="+", default=["0"])
    parser.add_argument("--out", default="results/pems04_622_kasa_core_ablation.csv")
    parser.add_argument("--markdown", default="results/pems04_622_kasa_core_ablation.md")
    ...
    summary_csv = ROOT / "results" / "pems04_622_kasa_core_ablation_summary.csv"
    ...
    if args.dry_run:
        ...
        for variant, cfg_path, seed in jobs:
            ...
            print(f"    overrides: {VARIANT_OVERRIDES[variant]}")
