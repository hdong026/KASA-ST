#!/usr/bin/env python3
"""Run KAN vs MLP prior-mapper seed ablation with parallel GPU workers."""
...
TEMP_CFG_DIR = os.path.join(ROOT, "tmp_configs", "prior_mapper_ablation")
RUN_LOG_DIR = os.path.join(ROOT, "logs", "prior_mapper_ablation")
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Compare history-prior KAN vs MLP mappers over multiple seeds."
    )
    parser.add_argument(
        "--cfg",
    ...
    parser.add_argument(
        ...
        help="Prior mapper types to compare.",
    )
    parser.add_argument(
        "--out",
        default=os.path.join(ROOT, "results", "pems04_prior_mapper_ablation.csv"),
        help="CSV output path.",
    )
    parser.add_argument(
        "--markdown",
        default=os.path.join(ROOT, "results", "pems04_prior_mapper_ablation.md"),
...
# ===== prior mapper ablation overrides (auto-generated) =====
CFG.MODEL.PARAM["prior_mapper_type"] = "{mapper}"
CFG.MODEL.PARAM["use_template_lookup"] = False
CFG.ENV.SEED = {seed}
if hasattr(CFG, "SEED"):
    CFG.SEED = {seed}
if hasattr(CFG, "TRAIN") and hasattr(CFG.TRAIN, "SEED"):
    CFG.TRAIN.SEED = {seed}
CFG.TRAIN.CKPT_SAVE_DIR = CFG.TRAIN.CKPT_SAVE_DIR + "_prior_{mapper}_seed{seed}"
"""
...
def load_ckpt_dir_from_cfg(cfg_path: str) -> str:
    spec = importlib.util.spec_from_file_location("ablation_cfg", cfg_path)
...
def run_single(
    ...
    print(f"[{mapper} seed={seed} gpu={gpu}] {cmd_str}")
    if dry_run:
...
def write_markdown(path: str, results: Sequence[RunResult], seeds: Sequence[int]) -> None:
    ...
    lines = ["# Prior Mapper Seed Ablation", ""]

    lines.append("## Per-run results")
    lines.append("")
    lines.append("| mapper | seed | MAE | RMSE | MAPE | status |")
    lines.append("| --- | ---: | ---: | ---: | ---: | --- |")
    for r in sorted(results, key=lambda x: (x.mapper, x.seed)):
        lines.append(
            f"| {r.mapper} | {r.seed} | {fmt(r.mae)} | {fmt(r.rmse)} | {fmt(r.mape)} | {r.status} |"
        )
    lines.append("")
    ...
    if not shared_seeds:
        ...
        for metric in ("mae", "rmse", "mape"):
            ...
            diff_rows.append((metric.upper(), mean, std, ci95(diffs)))
        lines.append("| metric | mean diff | std diff | 95% CI |")
        lines.append("| --- | ---: | ---: | ---: |")
        for metric, mean, std, ci in diff_rows:
            lines.append(f"| {metric} | {fmt(mean)} | {fmt(std)} | {fmt(ci)} |")
...
def main() -> None:
    ...
    write_csv(args.out, all_results)
    write_markdown(args.markdown, all_results, args.seeds)
