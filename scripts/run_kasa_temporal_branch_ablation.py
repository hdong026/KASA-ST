#!/usr/bin/env python3
"""KASA temporal branch ablation on PeMS04 12→12."""
...
TEMP_CFG_DIR = ROOT / "tmp_configs" / "kasa_temporal_branch_ablation"
LOG_DIR = ROOT / "results" / "kasa_temporal_branch_ablation_logs"
CKPT_ROOT = ROOT / "checkpoints" / "kasa_temporal_branch_ablation"
...
VARIANT_BRANCHES: dict[str, dict[str, bool]] = {
    "full": {
    ...
    "w_o_patch": {
        "use_patch_branch": False,
        "use_downsample_branch": True,
        "use_linear_residual_branch": True,
    },
    "w_o_downsample": {
        "use_patch_branch": True,
        "use_downsample_branch": False,
        "use_linear_residual_branch": True,
    },
    "w_o_residual": {
        "use_patch_branch": True,
...
PAIRED_DIFFS = [
    ...
    ("w_o_downsample", "full"),
    ("w_o_residual", "full"),
    ("patch_downsample", "full"),
    ("patch_residual", "full"),
    ("downsample_residual", "full"),
    ("residual_only", "full"),
    ("patch_only", "full"),
    ("downsample_only", "full"),
]
...
def variant_overrides(variant: str) -> dict:
    if variant not in VARIANT_BRANCHES:
        raise ValueError(f"Unknown variant: {variant}")
    return {**SPATIAL_OVERRIDES, **VARIANT_BRANCHES[variant]}
...
def generate_temp_config(variant: str, seed: int) -> Path:
    ...
    lines = [
        "",
        "# ===== kasa_temporal_branch_ablation overrides (auto-generated) =====",
        f"CFG.ENV.SEED = {seed}",
        "if hasattr(CFG, 'SEED'):",
        f"    CFG.SEED = {seed}",
        "if hasattr(CFG, 'TRAIN') and hasattr(CFG.TRAIN, 'SEED'):",
        f"    CFG.TRAIN.SEED = {seed}",
        f'CFG.TRAIN.CKPT_SAVE_DIR = os.path.join("checkpoints", "kasa_temporal_branch_ablation", "{variant}_seed{seed}")',
...
def write_outputs(rows: list[dict], out_csv: Path, out_md: Path, summary_csv: Path):
    ...
    md = [
        "# KASA Temporal Branch Ablation on PeMS04 12→12\n\n",
        "Protocol: official 6:2:2, `FORWARD=[0,1,2,3]`, `TARGET=[0]`, "
        "`post_spatial_mode=adaptive_only`, no prior residual, no pre-temporal spatial enhancement.\n\n",
        "## Per-run results\n\n",
...
def dry_run_info(variant: str, seed: int, cfg_path: Path) -> None:
    ...
    print(f"    use_patch_branch: {overrides['use_patch_branch']}")
    print(f"    use_downsample_branch: {overrides['use_downsample_branch']}")
    print(f"    use_linear_residual_branch: {overrides['use_linear_residual_branch']}")
    print(f"    post_spatial_mode: {overrides['post_spatial_mode']}")
...
def main() -> int:
    parser = argparse.ArgumentParser(description="KASA temporal branch ablation on PeMS04.")
    parser.add_argument("--variants", nargs="+", default=DEFAULT_VARIANTS,
                        choices=list(VARIANT_BRANCHES.keys()))
    parser.add_argument("--seeds", type=int, nargs="+", default=DEFAULT_SEEDS)
    parser.add_argument("--gpus", nargs="+", default=["0"])
    parser.add_argument("--out", default="results/pems04_kasa_temporal_branch_ablation.csv")
    parser.add_argument("--markdown", default="results/pems04_kasa_temporal_branch_ablation.md")
    ...
    summary_csv = ROOT / "results" / "pems04_kasa_temporal_branch_ablation_summary.csv"
