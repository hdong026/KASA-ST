#!/usr/bin/env python3
"""Run KASA v3 frequency-guided graph ablation on PeMS04."""
...
BASE_CFG_V2 = ROOT / "examples" / "KASAST_v2" / "KASAST_PEMS04.py"
BASE_CFG_V3 = ROOT / "examples" / "KASAST_v3" / "KASAST_v3_PEMS04.py"
RUN_PY = ROOT / "examples" / "run.py"
TEMP_CFG_DIR = ROOT / "tmp_configs" / "kasa_v3_freq_graph"
LOG_DIR = ROOT / "results" / "kasa_v3_freq_graph_ablation_logs"
CKPT_ROOT = ROOT / "checkpoints" / "kasa_v3_freq_graph"

DEFAULT_VARIANTS = ["v2_clean", "v3_freq_graph", "v3_cross_gate", "v3_spectral_gate"]
...
PAIRED_SECTIONS = [
    ("v3_freq_graph", "v2_clean", "v3_freq_graph - v2_clean", "v3_freq_graph is worse than v2_clean"),
    ("v3_cross_gate", "v2_clean", "v3_cross_gate - v2_clean", "v3_cross_gate is worse than v2_clean"),
...
def variant_report(variant: str) -> dict:
    ...
    base = overrides.get("base", "v3")
    report = {
        "model": "KASA_v2" if base == "v2" else "KASA_v3",
        "use_pre_temporal_spatial_enhancement": overrides.get(
            "use_pre_temporal_spatial_enhancement", False
        ),
        "keep_output_prior_residual": overrides.get("keep_output_prior_residual", False),
        "use_frequency_guided_graph": overrides.get("use_frequency_guided_graph", False),
        "use_cross_st_gate": overrides.get("use_cross_st_gate", False),
        "use_spectral_decomp_gate": overrides.get("use_spectral_decomp_gate", False),
...
def generate_temp_config(variant: str, seed: int) -> Path:
    ...
    overrides = VARIANT_OVERRIDES[variant]
    base = overrides.get("base", "v3")
    base_cfg = BASE_CFG_V2 if base == "v2" else BASE_CFG_V3
    content = strip_hardcoded_cuda_devices(base_cfg.read_text(encoding="utf-8"))
    lines = [
        "",
        "# ===== kasa_v3_freq_graph_ablation overrides (auto-generated) =====",
        f"CFG.ENV.SEED = {seed}",
        "if hasattr(CFG, 'SEED'):",
        f"    CFG.SEED = {seed}",
        "if hasattr(CFG, 'TRAIN') and hasattr(CFG.TRAIN, 'SEED'):",
        f"    CFG.TRAIN.SEED = {seed}",
        f'CFG.TRAIN.CKPT_SAVE_DIR = os.path.join("checkpoints", "kasa_v3_freq_graph", "{variant}_seed{seed}")',
...
def write_outputs(rows: list[dict], out_csv: Path, out_md: Path, summary_csv: Path):
    ...
    md = [
        "# KASA v3 Frequency Graph Ablation on PeMS04 12→12\n\n",
        "## Per-run results\n\n",
...
def main() -> int:
    parser = argparse.ArgumentParser(description="KASA v3 frequency graph ablation on PeMS04.")
    parser.add_argument("--variants", nargs="+", default=DEFAULT_VARIANTS,
                        choices=list(VARIANT_OVERRIDES.keys()))
    parser.add_argument("--seeds", type=int, nargs="+", default=DEFAULT_SEEDS)
    parser.add_argument("--gpus", nargs="+", default=["0"])
    parser.add_argument("--out", default="results/pems04_kasa_v3_freq_graph.csv")
    parser.add_argument("--markdown", default="results/pems04_kasa_v3_freq_graph.md")
    parser.add_argument("--dry_run", action="store_true")
    parser.add_argument("--summary-only", "--summary_only", action="store_true", dest="summary_only")
    args = parser.parse_args()

    if not BASE_CFG_V2.is_file():
        print(f"Missing base config: {BASE_CFG_V2}")
        return 1
    if not BASE_CFG_V3.is_file():
        print(f"Missing base config: {BASE_CFG_V3}")
        return 1

    summary_csv = ROOT / "results" / "pems04_kasa_v3_freq_graph_summary.csv"
