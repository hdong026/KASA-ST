#!/usr/bin/env python3
"""C2F coarse-to-fine ablation on PeMS04 12→12."""
...
BASE_CFG = ROOT / "examples" / "C2F" / "C2F_PEMS04.py"
RUN_PY = ROOT / "examples" / "run.py"
TEMP_CFG_DIR = ROOT / "tmp_configs" / "c2f_pems04_ablation"
LOG_DIR = ROOT / "results" / "c2f_pems04_ablation_logs"
CKPT_ROOT = ROOT / "checkpoints" / "c2f_pems04_ablation"

DEFAULT_VARIANTS = [
    "kasa_equivalent",
    "c2f_fc3",
    "c2f_fc6",
    "c2f_fc3_no_linear_residual",
]
...
PAIRED_DIFFS = [
    ("c2f_fc3", "kasa_equivalent"),
    ("c2f_fc6", "kasa_equivalent"),
    ("c2f_fc3_no_linear_residual", "c2f_fc3"),
    ("c2f_fc6", "c2f_fc3"),
]
...
def generate_temp_config(variant: str, seed: int) -> Path:
    ...
    lines = [
        ...
        f'CFG.TRAIN.CKPT_SAVE_DIR = os.path.join("checkpoints", "c2f_pems04_ablation", "{variant}_seed{seed}")',
...
def write_outputs(rows: list[dict], out_csv: Path, out_md: Path, summary_csv: Path):
    ...
    md = [
        "# C2F-KASA Ablation on PeMS04 12→12\n\n",
        "Protocol:\n",
        "official 6:2:2,\n",
        "TARGET=[0],\n",
...
def dry_run_info(variant: str, seed: int, cfg_path: Path) -> None:
    ...
    print(f"    model: C2F")
    print(f"    c2f_mode: {spec.get('c2f_mode', 'none')}")
    print(f"    coarse_len: {spec.get('coarse_len', 3)}")
    print(f"    use_linear_residual_in_c2f: {spec.get('use_linear_residual_in_c2f', True)}")
    print(f"    use_coarse_loss: {spec.get('use_coarse_loss', False)}")
    print(f"    coarse_loss_weight: {spec.get('coarse_loss_weight', 0.1)}")
...
def main() -> int:
    parser = argparse.ArgumentParser(description="C2F coarse-to-fine ablation on PeMS04.")
    parser.add_argument("--variants", nargs="+", default=DEFAULT_VARIANTS, choices=list(VARIANT_SPECS.keys()))
    parser.add_argument("--seeds", type=int, nargs="+", default=DEFAULT_SEEDS)
    parser.add_argument("--gpus", nargs="+", default=["0"])
    parser.add_argument("--out", default="results/pems04_c2f_ablation.csv")
    parser.add_argument("--markdown", default="results/pems04_c2f_ablation.md")
