...
# 尝试导入，如果没有也不影响主逻辑
try:
    from generate_adj_mx import generate_adj_knowair
except ImportError:
    pass
...
def generate_data(args: argparse.Namespace):
    ...
    # 1. Read data (KnowAir.npy: [T, N, C], target = last channel PM2.5)
    loaded = np.load(data_file_path)
    if isinstance(loaded, np.lib.npyio.NpzFile):
        arr = loaded["data"] if "data" in loaded.files else loaded[loaded.files[0]]
    else:
        arr = loaded
    if arr.ndim != 3:
        raise ValueError(f"Expected KnowAir array [T, N, C], got shape {arr.shape}")
    # support target_channel like [-1] or [17]
    ch = target_channel[0] if isinstance(target_channel, (list, tuple)) else int(target_channel)
    data = arr[..., ch][..., np.newaxis].astype(np.float32)
    ...
    if os.path.exists(args.graph_file_path):
        dst_adj = output_dir + "/adj_mx.pkl"
        try:
            if os.path.realpath(args.graph_file_path) != os.path.realpath(dst_adj):
                shutil.copyfile(args.graph_file_path, dst_adj)
        except Exception as e:
            print("Warning: adj copy skipped:", e)
    else:
        try:
            raise RuntimeError("KnowAir adj missing")
...
if __name__ == "__main__":
    ...
    TRAIN_RATIO = 0.6
    VALID_RATIO = 0.2
    TARGET_CHANNEL = [-1]  # PM2.5 last channel
    STEPS_PER_DAY = 8
    ...
    DATA_FILE_PATH = os.path.join(project_root, "datasets", "raw_data", DATASET_NAME, f"{DATASET_NAME}.npy")
