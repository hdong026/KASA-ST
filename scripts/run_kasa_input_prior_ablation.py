#!/usr/bin/env python3
"""Run KASA input-prior enhancement ablations on PeMS04."""
...
TEMP_CFG_DIR = ROOT / "tmp_configs" / "kasa_input_prior_ablation"
LOG_DIR = ROOT / "results" / "kasa_input_prior_ablation_logs"
CKPT_ROOT = ROOT / "checkpoints" / "kasa_input_prior_ablation"

DEFAULT_VARIANTS = ["current_best", "input_only", "input_plus_output"]
...
VARIANT_OVERRIDES: dict[str, dict] = {
    ...
    "input_only": {
        "prior_mapper_type": "mlp",
        "use_pre_temporal_spatial_enhancement": False,
        "use_input_prior_enhancement": True,
...
def variant_report(variant: str) -> dict:
    ...
    report = {
        ...
        "input_prior_mapper_type": overrides.get("input_prior_mapper_type", "mlp"),
...
def generate_temp_config(variant: str, seed: int) -> Path:
    ...
    lines = [
        "",
        "# ===== kasa_input_prior_ablation overrides (auto-generated) =====",
        f"CFG.ENV.SEED = {seed}",
        "if hasattr(CFG, 'SEED'):",
        f"    CFG.SEED = {seed}",
        "if hasattr(CFG, 'TRAIN') and hasattr(CFG.TRAIN, 'SEED'):",
        f"    CFG.TRAIN.SEED = {seed}",
        f'CFG.TRAIN.CKPT_SAVE_DIR = os.path.join("checkpoints", "kasa_input_prior_ablation", "{variant}_seed{seed}")',
...
def write_outputs(rows: list[dict], out_csv: Path, out_md: Path, summary_csv: Path):
    ...
    md = [
        "# PeMS04 KASA Input Prior Ablation\n\n",
        "Protocol: `FORWARD=[0,1,2,3]`, `TARGET=[0]`.\n\n",
        "## Per-run results\n\n",
...
def main() -> int:
    parser = argparse.ArgumentParser(description="KASA input-prior enhancement ablation on PeMS04.")
    parser.add_argument("--variants", nargs="+", default=DEFAULT_VARIANTS,
                        choices=list(VARIANT_OVERRIDES.keys()))
    parser.add_argument("--seeds", type=int, nargs="+", default=DEFAULT_SEEDS)
    parser.add_argument("--gpus", nargs="+", default=["0"])
    parser.add_argument("--out", default="results/pems04_kasa_input_prior_ablation.csv")
    parser.add_argument("--markdown", default="results/pems04_kasa_input_prior_ablation.md")
    ...
    summary_csv = ROOT / "results" / "pems04_kasa_input_prior_ablation_summary.csv"
