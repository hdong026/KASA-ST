#!/usr/bin/env python3
"""Protocol A: fixed input H=12, variable forecast F∈{12,24,48} on PeMS04."""
...
LOG_DIR = ROOT / "results" / "fixed_input_horizon_pems04_logs"
...
VARIANT_SPECS: dict[str, dict] = {
    ...
    # Spatial sensitivity: Strong (same final adapter/token-loss; stronger progressive spatial)
...
def generate_temp_config(horizon: int, variant: str, seed: int) -> Path:
    ...
    ckpt_rel = os.path.join("checkpoints", "fixed_input_horizon_pems04", f"h{horizon}", f"{variant}_seed{seed}")
    lines = [
        "",
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Protocol A fixed-input horizon ablation on PeMS04.")
    ...
    parser.add_argument("--out", default="results/pems04_fixed_input_horizon.csv")
    parser.add_argument("--markdown", default="results/pems04_fixed_input_horizon.md")
