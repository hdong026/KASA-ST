#!/usr/bin/env python3
"""Run KASA v3-freqgate ablation on PeMS04."""
...
BASE_CFG_V3FG = ROOT / "examples" / "KASAST_v3_freqgate" / "KASAST_v3_freqgate_PEMS04.py"
RUN_PY = ROOT / "examples" / "run.py"
TEMP_CFG_DIR = ROOT / "tmp_configs" / "kasa_v3_freqgate"
LOG_DIR = ROOT / "results" / "kasa_v3_freqgate_ablation_logs"
CKPT_ROOT = ROOT / "checkpoints" / "kasa_v3_freqgate"
...
def variant_report(variant: str) -> dict:
    ...
    report = {
        ...
        "use_input_prior_enhancement": overrides.get("use_input_prior_enhancement", False),
        "FORWARD_FEATURES": FORWARD_FEATURES,
        "TARGET_FEATURES": TARGET_FEATURES,
    }
    if base == "v3_freqgate":
        report.update({
            "use_frequency_guided_graph": overrides.get("use_frequency_guided_graph", True),
            "use_freq_conditioned_fusion": overrides.get("use_freq_conditioned_fusion", True),
            "use_cross_st_gate": overrides.get("use_cross_st_gate", True),
            "use_spectral_decomp_gate": overrides.get("use_spectral_decomp_gate", False),
        })
    else:
        report.update({
            "use_frequency_guided_graph": "n/a",
            "use_freq_conditioned_fusion": "n/a",
            "use_cross_st_gate": "n/a",
            "use_spectral_decomp_gate": "n/a",
        })
    return report
...
def generate_temp_config(variant: str, seed: int) -> Path:
    ...
    overrides = VARIANT_OVERRIDES[variant]
    base = overrides.get("base", "v3_freqgate")
    base_cfg = BASE_CFG_V2 if base == "v2" else BASE_CFG_V3FG
    ...
    lines = [
        ...
        f'CFG.TRAIN.CKPT_SAVE_DIR = os.path.join("checkpoints", "kasa_v3_freqgate", "{variant}_seed{seed}")',
...
def write_outputs(rows: list[dict], out_csv: Path, out_md: Path, summary_csv: Path):
    ...
    md = [
        "# KASA v3-freqgate Ablation on PeMS04 12→12\n\n",
        "## Per-run results\n\n",
...
def main() -> int:
    parser = argparse.ArgumentParser(description="KASA v3-freqgate ablation on PeMS04.")
    parser.add_argument("--variants", nargs="+", default=DEFAULT_VARIANTS,
                        choices=list(VARIANT_OVERRIDES.keys()))
    parser.add_argument("--seeds", type=int, nargs="+", default=DEFAULT_SEEDS)
    parser.add_argument("--gpus", nargs="+", default=["0"])
    parser.add_argument("--out", default="results/pems04_kasa_v3_freqgate.csv")
    parser.add_argument("--markdown", default="results/pems04_kasa_v3_freqgate.md")
    parser.add_argument("--dry_run", action="store_true")
    parser.add_argument("--summary-only", "--summary_only", action="store_true", dest="summary_only")
    args = parser.parse_args()

    if not BASE_CFG_V2.is_file() or not BASE_CFG_V3FG.is_file():
        print(f"Missing base config(s): {BASE_CFG_V2} / {BASE_CFG_V3FG}")
        return 1

    summary_csv = ROOT / "results" / "pems04_kasa_v3_freqgate_summary.csv"
