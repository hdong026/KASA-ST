#!/usr/bin/env python3
"""KASA patch data input ablation on PeMS04 12→12."""
...
TEMP_CFG_DIR = ROOT / "tmp_configs" / "kasa_patch_input_ablation"
LOG_DIR = ROOT / "results" / "kasa_patch_input_ablation_logs"
CKPT_ROOT = ROOT / "checkpoints" / "kasa_patch_input_ablation"

DEFAULT_VARIANTS = ["base_all", "patch_flow_only"]
...
def generate_temp_config(variant: str, seed: int) -> Path:
    ...
    lines = [
        "",
        "# ===== kasa_patch_input_ablation overrides (auto-generated) =====",
        f"CFG.ENV.SEED = {seed}",
        "if hasattr(CFG, 'SEED'):",
        f"    CFG.SEED = {seed}",
        "if hasattr(CFG, 'TRAIN') and hasattr(CFG.TRAIN, 'SEED'):",
        f"    CFG.TRAIN.SEED = {seed}",
        f'CFG.TRAIN.CKPT_SAVE_DIR = os.path.join("checkpoints", "kasa_patch_input_ablation", "{variant}_seed{seed}")',
...
def write_outputs(rows: list[dict], out_csv: Path, out_md: Path, summary_csv: Path):
    ...
    md = [
        "# KASA Patch Input Ablation on PeMS04 12→12\n\n",
        "Protocol:\n",
        "official 6:2:2,\n",
...
def dry_run_info(variant: str, seed: int, cfg_path: Path) -> None:
    ...
    print(f"    patch_data_input_mode: {spec['patch_data_input_mode']}")
...
def main() -> int:
    parser = argparse.ArgumentParser(description="KASA patch data input ablation on PeMS04.")
    parser.add_argument("--variants", nargs="+", default=DEFAULT_VARIANTS, choices=list(VARIANT_SPECS.keys()))
    parser.add_argument("--seeds", type=int, nargs="+", default=DEFAULT_SEEDS)
    parser.add_argument("--gpus", nargs="+", default=["0"])
    parser.add_argument("--out", default="results/pems04_kasa_patch_input_ablation.csv")
    parser.add_argument("--markdown", default="results/pems04_kasa_patch_input_ablation.md")
