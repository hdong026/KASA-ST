#!/usr/bin/env python3
"""ST-FSF horizon ablation on PeMS04 (H=12, F∈{12,24,48})."""
...
ST_FSF_BASE_CFG = ROOT / "examples" / "STForecastStateFlow" / "STForecastStateFlow_PEMS04.py"
RUN_PY = ROOT / "examples" / "run.py"
PREPARE_SCRIPT = ROOT / "scripts" / "prepare_pems04_fixed_input_horizons.py"
TEMP_CFG_DIR = ROOT / "tmp_configs" / "st_forecast_state_flow_horizon_pems04"
LOG_DIR = ROOT / "results" / "st_forecast_state_flow_horizon_pems04_logs"
CKPT_ROOT = ROOT / "checkpoints" / "st_forecast_state_flow_horizon_pems04"
...
VARIANT_SPECS: dict[str, dict] = {
    "st_fsf_flow": {
        ...
        "use_projection_after_flow": True,
        "q_list_override": None,
        "num_flow_steps": 1,
    ...
    "st_fsf_no_projection": {
        "direct_forecast": False,
        "use_projection_after_flow": False,
        "q_list_override": None,
        "num_flow_steps": 1,
        "final_loss_weight": 1.0,
        "state_loss_weight": 0.3,
...
def load_cfg(cfg_path: Path):
    spec = importlib.util.spec_from_file_location("st_fsf_horizon_cfg", cfg_path)
...
def q_values(node_size: int = NODE_SIZE) -> tuple[int, int]:
    return math.ceil(node_size * 0.25), math.ceil(node_size * 0.50)


def stage_specs_for(horizon: int, variant: str, node_size: int = NODE_SIZE) -> list[tuple[int, int]]:
    if str(ROOT) not in sys.path:
        sys.path.insert(0, str(ROOT))
    from basicts.archs import STForecastStateFlow

    spec = VARIANT_SPECS[variant]
    q_override = spec.get("q_list_override")
    if q_override == "full":
        q_override = [node_size, node_size, node_size]
    return STForecastStateFlow.build_stage_specs(
        output_len=horizon,
        node_size=node_size,
        direct=spec.get("direct_forecast", False),
        q_list_override=q_override,
    )
...
def variant_spec(variant: str, horizon: int) -> dict:
    ...
    spec["q1"] = q1
    spec["q2"] = q2
    spec["stage_specs"] = stage_specs_for(horizon, variant)
    if spec.get("q_list_override") == "full":
        spec["q_list_override"] = [NODE_SIZE, NODE_SIZE, NODE_SIZE]
    return spec
...
def generate_temp_config(horizon: int, variant: str, seed: int) -> Path:
    ...
    content = strip_hardcoded_cuda_devices(ST_FSF_BASE_CFG.read_text(encoding="utf-8"))
    ckpt_rel = os.path.join(
        "checkpoints", "st_forecast_state_flow_horizon_pems04", f"h{horizon}", f"{variant}_seed{seed}"
    )
    lines = [
        "",
        "# ===== st_forecast_state_flow_horizon_pems04 overrides (auto-generated) =====",
        f"CFG.ENV.SEED = {seed}",
        "if hasattr(CFG, 'SEED'):",
        f"    CFG.SEED = {seed}",
...
def verify_model_shapes(cfg_path: Path) -> tuple[bool, str]:
    try:
        ...
        model = STForecastStateFlow(**param)
        ...
        specs = out["stage_specs"]
        for i, s in enumerate(out["states"]):
            if tuple(s.shape) != (b, f_len, n, 1):
                return False, f"state[{i}] shape {tuple(s.shape)}"
        for i, (ns, (r, q)) in enumerate(zip(out["native_states"], specs)):
            if tuple(ns.shape) != (b, r, q, 1):
                return False, f"native_state[{i}] shape {tuple(ns.shape)} expected (B,{r},{q},1)"

        fm_count = len(out.get("fm_items", []))
        expected_fm = max(len(specs) - 1, 0)
        if fm_count != expected_fm:
            return False, f"fm_items={fm_count}, expected={expected_fm}"

        basis_once = model.graph_basis is not None
        return True, f"pred={tuple(pred.shape)} specs={specs} fm={fm_count} basis={basis_once}"
...
def base_row(horizon: int, variant: str, seed: int, cfg_path: Path) -> dict:
    ...
    return {
        ...
        "input_len": INPUT_LEN,
        "output_len": horizon,
        "stage_specs": str(spec["stage_specs"]),
        "q1": spec["q1"],
        "q2": spec["q2"],
        ...
        "use_projection_after_flow": spec.get("use_projection_after_flow"),
        "mae": None,
        "rmse": None,
        "mape": None,
        "params": params,
...
def write_outputs(rows: list[dict], out_csv: Path, out_md: Path, summary_csv: Path) -> None:
    ...
    md = [
        "# ST-FSF Horizon Ablation on PeMS04\n\n",
        "Fixed input H=12; output F∈{12,24,48}. ST chain: (r/4,q/4) → (r/2,q/2) → (F,N).\n\n",
        "## Per-run results\n\n",
        "| horizon | variant | seed | input | output | stage_specs | q1 | q2 | flow_steps | "
        "fm_w | native_w | state_w | proj | MAE | RMSE | MAPE | params | FLOPs | inference_time | failed |\n",
        "|---:|---|---:|---:|---:|---|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|\n",
    ]
    for r in rows:
        md.append(
            f"| {r['horizon']} | {r['variant']} | {r['seed']} | {r['input_len']} | {r['output_len']} | "
            f"{r['stage_specs']} | {r['q1']} | {r['q2']} | {fmt_val(r.get('num_flow_steps'))} | "
            f"{fmt_val(r.get('fm_loss_weight'))} | {fmt_val(r.get('native_loss_weight'))} | "
            f"{fmt_val(r.get('state_loss_weight'))} | {fmt_val(r.get('use_projection_after_flow'))} | "
...
def verify_batch_shape(horizon: int) -> tuple[bool, str]:
    ...
    if tuple(history_data.shape[1:3]) != (INPUT_LEN, NODE_SIZE):
...
def dry_run_info(horizon: int, variant: str, seed: int, cfg_path: Path) -> None:
    ...
    print(f"    stage_specs: {spec['stage_specs']}")
    print(f"    q1={spec['q1']} q2={spec['q2']}")
    print(f"    use_projection_after_flow: {spec.get('use_projection_after_flow')}")
    print(f"    fm_loss_weight: {spec.get('fm_loss_weight')}")
    print(f"    params: {count_params(cfg_path)}")
    print(f"    graph_basis: datasets/PEMS04/adj_mx.pkl (fixed, init-time)")
...
def main() -> int:
    parser = argparse.ArgumentParser(description="ST-FSF horizon ablation on PeMS04.")
    ...
    parser.add_argument("--out", default="results/pems04_st_fsf_horizon.csv")
    parser.add_argument("--markdown", default="results/pems04_st_fsf_horizon.md")
    ...
    if not ST_FSF_BASE_CFG.is_file():
        print(f"Missing base config: {ST_FSF_BASE_CFG}")
        return 1
    ...
    if args.dry_run:
        ...
        for horizon, variant, cfg_path, seed in jobs:
            ...
            cfg = load_cfg(cfg_path)
            expected = stage_specs_for(horizon, variant)
            model = cfg.MODEL.ARCH(**cfg.MODEL.PARAM)
            if list(model.stage_specs) != expected:
                print(f"    ERROR: stage_specs {model.stage_specs} != {expected}")
                all_ok = False
            shape_ok, _ = verify_model_shapes(cfg_path)
            if not shape_ok:
                all_ok = False
        print(f"\n{len(jobs)} jobs (expected {expected_jobs}), unique ckpt dirs: {len(ckpt_dirs)}")
        print(f"ST-FSF ckpt root: {CKPT_ROOT}")
        print(f"Chain ckpt (untouched): {ROOT / 'checkpoints' / 'fixed_input_horizon_pems04'}")
        for h in args.horizons:
