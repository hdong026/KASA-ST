...
class G1S12CapDistFinalPrimaryRunner(ChainForecastingRunner):
    """G1 final-primary grad surgery + single S1/2 capdist graph stage before S1 final."""

    GRAPH_AUX_LOSS_NAMES = ("L_G12",)
    ...
    def __init__(self, cfg: dict):
        ...
        self.variant_name = str(param.get("variant_name", "G1_S12_capdist_final_primary"))
        self.base_variant = str(param.get("base_variant", "G1_final_primary_grad_surgery"))
        if not self.final_primary_grad_surgery:
            raise ValueError(
                "G1S12CapDistFinalPrimaryRunner requires final_primary_grad_surgery=True"
            )
        if self.stagewise_enabled:
            raise ValueError("G1_S12_capdist_final_primary does not support stagewise training")
        if self.unified_aux_loss_mode != "none":
            raise ValueError("G1_S12_capdist_final_primary requires unified_aux_loss_mode='none'")
        placement = str(param.get("spatial_placement", "")).lower()
        if placement != "temporal_first_graph_resolution":
            raise ValueError(
                "G1_S12_capdist_final_primary requires spatial_placement="
                "'temporal_first_graph_resolution'"
            )
        post_mode = str(param.get("post_spatial_mode", "")).lower()
        if post_mode != "adaptive_cluster_mix":
            raise ValueError(
                "G1_S12_capdist_final_primary requires post_spatial_mode='adaptive_cluster_mix'"
            )
        capacities = list(param.get("graph_resolution_capacities") or [])
        if capacities and len([c for c in capacities if int(c) > 1]) > 1:
            raise ValueError(
                "G1_S12_capdist_final_primary allows at most one coarse graph stage (no S1/4)"
            )
        ...
        self._aux_loss_names = ["L_T1", "L_T2", *list(self.GRAPH_AUX_LOSS_NAMES)]

    def init_training(self, cfg: dict):
        super().init_training(cfg)
        self.logger.info(
            "variant=%s base=%s graph_resolution=S1/2->S1 weighted_sum_loss=disabled "
            "final_primary_grad_projection=enabled temporal_aux=enabled "
            "graph_aux=L_G12_only S1/4=disabled aux_grad_max_ratio=%.2f",
            self.variant_name,
            self.base_variant,
    ...
    def _resolve_graph_aux(
        ...
        if not graph_stage_preds:
            raise RuntimeError(
                "G1_S12_capdist_final_primary requires graph_stage_preds from forward"
            )
        if not projection_matrices:
            raise RuntimeError(
                "G1_S12_capdist_final_primary requires graph_projection_matrices from forward"
            )
        return {
            "L_G12": self._graph_stage_projected_loss(
                graph_stage_preds[0],
                target_full,
    ...
    def on_epoch_end(self, epoch: int):
        ...
        if not stats["L_final"]:
            return
        self.logger.info(
            "[%s epoch=%s] L_final=%.4f L_T1=%.4f L_T2=%.4f L_G12=%.4f "
            "cos_T1_final=%.4f cos_T2_final=%.4f cos_G12_final=%.4f "
            "aux_grad_norm/final_grad_norm=%.4f val_MAE=%.4f test_MAE@best-val=%.4f",
            self.variant_name,
            epoch,
    ...
    def train_iters(
        ...
        if pred_final is None or pred_t_low is None or pred_t_mid is None:
            raise RuntimeError(
                "G1_S12_capdist_final_primary requires pred, pred_T_low, pred_T_mid from forward"
            )
        ...
        graph_losses = self._resolve_graph_aux(out, target_full)
