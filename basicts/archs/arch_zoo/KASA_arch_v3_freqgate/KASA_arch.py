"""KASA v3-freqgate: frequency-guided spatial activation after temporal backbone."""
...
from basicts.archs.arch_zoo.KASA_arch_v3_freqgate.patch_emb import PatchEncoder
from basicts.archs.arch_zoo.KASA_arch_v3_freqgate.downsamp_emb import DownsampEncoder
from basicts.archs.arch_zoo.KASA_arch_v3_freqgate.gcn import FreqGateSpatialModule
...
class KASA_v3_FreqGate(nn.Module):
    def __init__(self, **model_args):
        ...
        self.spatial_module = FreqGateSpatialModule(
            node_size=self.node_size,
            input_len=self.input_len,
            d_spa=self.d_spa,
            if_spatial=self.if_spatial,
            adj_mx_path=model_args.get("adj_mx_path"),
            use_gcn=model_args.get("use_gcn", True),
            ...
            freq_dim=model_args.get("freq_dim", 16),
            freq_topk=model_args.get("freq_topk", 20),
            freq_eps=model_args.get("freq_eps", 1e-6),
            use_frequency_guided_graph=model_args.get("use_frequency_guided_graph", True),
            use_freq_conditioned_fusion=model_args.get("use_freq_conditioned_fusion", True),
            graph_fusion_hidden=model_args.get("graph_fusion_hidden", 16),
            use_cross_st_gate=model_args.get("use_cross_st_gate", True),
            gate_hidden=model_args.get("gate_hidden", 16),
            gate_residual_scale=model_args.get("gate_residual_scale", 1.0),
            use_spectral_decomp_gate=model_args.get("use_spectral_decomp_gate", False),
        )

        encoder_input_dim = 3
        self.patch_encoder = PatchEncoder(
            self.td_size, self.dw_size, self.td_codebook, self.dw_codebook, self.spa_codebook,
        ...
        if self.use_input_prior_enhancement:
            if self.input_dim <= 3:
                raise ValueError(
                    "use_input_prior_enhancement=True requires input_dim > 3 (channel 3 prior)"
                )
            mapper_type = model_args.get("input_prior_mapper_type", "mlp")
            if mapper_type == "mlp":
                ...
                raise ValueError(f"Unsupported input_prior_mapper_type: {mapper_type}")
    ...
    def forward(
        ...
        **kwargs,
        ...
        patch_predict = self.patch_encoder(patch_input, spatial_codebook=spatial_codebook_for_encoder)
        downsamp_predict = self.downsamp_encoder(
            downsamp_input, spatial_codebook=spatial_codebook_for_encoder
        )
        res_out = self.residual(history_data[..., 0:1].permute(0, 1, 2, 3))

        if kwargs.get("return_backbone", False):
            return patch_predict + downsamp_predict

        y_temporal = patch_predict + downsamp_predict + res_out
        history_flow = history_data[..., 0]
        y_final = self.spatial_module.refine_prediction(
            y_temporal,
        ...
        if self.keep_output_prior_residual:
            if self.input_dim <= 3:
                raise ValueError(
                    "keep_output_prior_residual=True requires input_dim > 3 (channel 3 prior)"
                )
            if self.prior_source == "history":
                prior_data = history_data[..., 3:4]
            elif self.prior_source == "future":
                if future_data is None or future_data.shape[-1] <= 3:
                    raise ValueError("future_data with channel 3 required for prior_source='future'")
                prior_data = future_data[..., 3:4]
            else:
                raise ValueError(f"Unknown prior_source: {self.prior_source}")
            y_final = y_final + self.prior_mapper(prior_data)
